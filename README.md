...
このリポジトリの公開正本は `v3.8-rc.1` です。現在の承認済み制作範囲は次の17単位です。
...
単なる用語暗記や、教材外の珍しい語形を当てる問題にはしません。解答後には理由と「持ち帰る一行」を表示します。第1〜8課の例文とTTS本文は維持し、第9〜16課も同じ説明・復習導線で新規制作しています。
...
- `curriculum-v38.js` — 現在の公開範囲を組み立てる入口
- `app-v28.js` — ナビ、進捗、練習、音声、印刷（UI本体を段階移行中）
- `styles-v28.css` — モバイル・印刷スタイル
- `manifest-v38.webmanifest` / `sw.js` — PWAとオフラインキャッシュ
...
第3〜16課を含む `confidence.pronunciation:C` の項目は設計上公開可能ですが、ネイティブ音声監査待ちです。`D` は公開できません。
...
- データ・転写・教材構造：13,634 assertions
- 厳密転写ゴールデンケース：66件
- 公開教材データ：330項目
- DOM・設定移行・音声イベント・16課描画：79 assertions
- 公開範囲：PART 0＋第1〜16課
