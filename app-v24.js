(() => {
  ...
  function renderNav() {
    ...
    const visibleChapters = book.chapters.map((chapter, index) => ({ chapter, index })).filter(({ chapter }) => {
      ...
      const searchable = [chapter.number, pad(chapter.number), chapter.navTitle, chapter.title, chapter.tamilTitle, chapter.deck, ...chapter.targets].join(" ").toLocaleLowerCase();
  ...
  function chapterPart(chapter) {
    ...
    return { code: "PART V", title: "文を伸ばす" };
  }
