(() => {
  ...
  function init() {
    ...
    if (!document.documentElement.dataset.standalone && "serviceWorker" in navigator && /^https?:$/.test(window.location.protocol)) {
      navigator.serviceWorker.register("sw-v26.js").catch(() => {});
    }
  }
  ...
  function updateBookLabels() {
    ...
    if (progressLabel) progressLabel.textContent = `第${state.chapterIndex + 1}章 / ${count}章`;
    ...
    if (printBookButton) printBookButton.textContent = `全${count}章を印刷`;
  }
  ...
  function render() {
    ...
    document.title = `第${chapter.number}章 ${chapter.navTitle}｜${book.meta.title}`;
    document.getElementById("progressLabel").textContent = `第${chapter.number}章 / ${book.chapters.length}章`;
    document.getElementById("progressFill").style.width = `${(chapter.number / book.chapters.length) * 100}%`;

    const formsButton = bottomNav.querySelector('button[data-view="forms"]');
    if (formsButton) {
      const formLabel = chapter.formConfig.mode === "vocabulary" ? "語彙" : chapter.formConfig.mode === "grammar" ? "文法" : "形";
      formsButton.innerHTML = `<span>03</span>${formLabel}`;
    }
  ...
  function renderHero(chapter) {
    ...
    return `
      <header class="chapter-hero">
        <span class="chapter-number">${part.code} · CHAPTER ${pad(chapter.number)}</span>
  ...
  function renderNote(note) {
    return `<aside class="note-card ${note.tone || ""}"><strong>${note.title}</strong>${note.body}</aside>`;
  }
  ...
  function renderExample(example, featured, id) {
    ...
    return `
      <article class="${featured ? "example-feature" : "example-row"}${dialogueClass}" data-example-id="${escapeHtml(resolvedId || "")}">
        ${playableId ? `<button class="play-button" type="button" data-play-id="${escapeHtml(playableId)}" aria-label="タミル語を再生">▶</button>` : ""}
        <div class="tamil-line" lang="ta">${example.ta}</div>
        <div class="roman-line morph">${example.morph || example.roman}</div>
        <div class="roman-line plain">${example.roman}</div>
        <div class="kana-line">${example.kana}</div>
  ...
  function renderGrammarCard(item) {
    return `<article class="grammar-card">
      <button class="word-audio" type="button" data-tts="${escapeAttr(item.ta)}" data-label="${escapeAttr(item.ja)}" aria-label="${escapeAttr(item.ta)}を再生">▶</button>
      <span class="grammar-kind">${escapeHtml(item.kind || "型")}</span>
      <strong lang="ta">${item.ta}</strong>
      <span class="grammar-roman morph-code">${item.roman}</span>
      <span class="kana-line">${item.kana}</span>
  ...
  function renderVocabularyCard(item) {
    return `<article class="vocab-card">
      ...
      <strong lang="ta">${item.ta}</strong>
      <span class="vocab-roman">${item.roman}</span>
      <span class="kana-line">${item.kana}</span>
  ...
  function renderFormCard(verb, config) {
    const rows = config.keys.map(key => {
      ...
      return `<tr>
        ...
        <td>
          <button class="word-audio" type="button" data-tts="${escapeAttr(form.ta)}" data-label="${escapeAttr(verb.meaning + "・" + config.labels[key])}" aria-label="${escapeAttr(form.ta)}を再生">▶</button>
          <span class="ta-form" lang="ta">${form.ta}</span>
          <span class="ro-form">${form.roman}</span>
          <span class="kana-line">${form.kana}</span>
        </td>
      </tr>`;
    }).join("");
    ...
    return `
      ...
      <div class="compare-table-wrap"><table class="compare-table">
        ...
        <tbody>${rows.map(row => `<tr>
          <td>${row.label}</td><td lang="ta">${row.ta}</td><td><span class="morph-code">${row.roman}</span><br><span class="kana-line">${row.kana}</span></td><td>${row.ja}</td>
          <td><button class="word-audio" type="button" data-tts="${escapeAttr(row.ta)}" data-label="${escapeAttr(row.label)}" aria-label="${escapeAttr(row.ta)}を再生">▶</button></td>
        </tr>`).join("")}</tbody>
      </table></div>`;
  }
  ...
  function renderNav() {
    ...
    const visibleChapters = book.chapters.map((chapter, index) => ({ chapter, index })).filter(({ chapter }) => {
      ...
      const searchable = [
        ...
        ...vocabulary.flatMap(item => [item.ta, item.roman, item.kana, item.ja, item.group, item.note]),
        ...patterns.flatMap(item => [item.ta, item.roman, item.kana, item.ja, item.group, item.kind, item.note])
      ].join(" ").toLocaleLowerCase();
      return searchable.includes(query);
    });

    chapterNav.innerHTML = visibleChapters.map(({ chapter, index }) => {
      const progress = getChapterProgress(chapter.id);
      ...
      return `${partHeading}<button class="chapter-nav-button ${index === state.chapterIndex ? "active" : ""} ${complete ? "complete" : ""}" type="button" data-chapter="${index}" aria-current="${index === state.chapterIndex ? "page" : "false"}" aria-label="第${chapter.number}章 ${escapeAttr(chapter.navTitle)}、${complete ? "完了" : `${completedSteps}/5ステップ`}">
    ...
    empty.hidden = visibleChapters.length > 0;
  ...
  function finishQuiz() {
    ...
    render();
    const score = calculateScore(chapter, quiz);
    showToast(`第${chapter.number}章 完了：${score}/${chapter.quiz.length}`);
  }
  ...
  function preparePrint(scope) {
    ...
    const chapters = scope === "book" ? book.chapters : [currentChapter()];
  ...
  function speakExample(example, preserveRepeat = false) {
    ...
    const utterance = new SpeechSynthesisUtterance(example.ta);
  ...
  function currentChapter() {
    return book.chapters[state.chapterIndex];
  }
  ...
  function updateHash() {
    const hash = `#ch=${state.chapterIndex + 1}&view=${state.view}`;
  ...
  function parseHash() {
    ...
    const chapter = Number.isFinite(chapterRaw) && chapterRaw > 0 ? chapterRaw - 1 : null;
    const view = params.get("view");
    return { chapter, view: VIEW_LABELS[view] ? view : null };
  }
