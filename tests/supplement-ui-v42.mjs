...
assert(Object.keys(state.progress).join(",") === "A,B", "only loaded supplement A and B progress should be created");
...
assert(ui.overallCompletion(state).percent === 50, "A complete and B untouched must be 50 percent overall");
...
assert(restored.currentCode === "B", "restored supplement selection mismatch");
...
assert(hub.includes("補講 A") && hub.includes("補講 B"), "hub must list A and B");
...
assert(!hub.includes("第21課") && !hub.includes("第22課"), "hub must not rename supplements as lessons 21 and 22");
...
assert(Object.keys(serialized.progress).join(",") === "A,B", "serialized progress must contain only A and B");
...
if (failures.length) {
  ...
  console.log(`v4.2 supplement UI validation passed: ${assertions} assertions`);
  console.log("- supplement progress uses an isolated storage key");
  console.log("- A/B progress and quizzes remain separate from the twenty core lessons");
