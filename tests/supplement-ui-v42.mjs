...
assert(Object.keys(state.progress).join(",") === "A,B,C,D,E,F", "loaded supplement A–F progress should be created");
...
for (const code of ["B", "C", "D", "E", "F"]) assert(ui.getProgress(state, code).read === false, `reading A must not mark ${code}.read`);
...
for (const code of ["B", "C", "D", "E", "F"]) assert(ui.completion(state, code).done === 0, `${code} must remain untouched`);
...
assert(ui.overallCompletion(state).percent === 17, "A complete and B–F untouched must round to 17 percent overall");
...
assert(["C", "D", "E", "F"].every(code => ui.getProgress(state, code).forms === false), "working on B must not mark C–F forms");
...
assert(restored.currentCode === "F", "restored supplement selection mismatch");
...
for (const code of ["C", "D", "E", "F"]) assert(restored.progress[code].read === true, `restored ${code} read progress missing`);

const hub = ui.renderHub(restored);
for (const code of ["A", "B", "C", "D", "E", "F"]) assert(hub.includes(`補講 ${code}`), `hub must list supplement ${code}`);
...
for (const number of [21, 22, 23, 24, 25, 26]) assert(!hub.includes(`第${number}課`), `hub must not rename a supplement as lesson ${number}`);
...
assert(Object.keys(serialized.progress).join(",") === "A,B,C,D,E,F", "serialized progress must contain A–F");
...
if (failures.length) {
  ...
  console.log("- A/B/C/D/E/F progress and quizzes remain separate from the twenty core lessons");
